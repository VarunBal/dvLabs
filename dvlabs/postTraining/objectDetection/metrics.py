...
def ap_per_class(tp, conf, pred_cls, target_cls, plot=False, save_dir='.', names=(), eps=1e-16, prefix=""):
    ...
    f1 = 2 * p * r / (p + r + eps)
    names = unique_classes  # list: only classes that have data
    ...
    if CIoU or DIoU or GIoU:
        ...
        if CIoU or DIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            if CIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
                v = (4 / math.pi ** 2) * (np.arctan(w2 / h2) - np.arctan(w1 / h1)).pow(2)
                alpha = v / (v - iou + (1 + eps))
...
def wh_iou(wh1, wh2, eps=1e-7):
    ...
    wh1 = wh1[:, None]  # [N,1,2]
    wh2 = wh2[None]  # [1,M,2]
    inter = np.min(wh1, wh2).prod(2)  # [N,M]
...
def plot_pr_curve(px, py, ap, save_dir=Path('pr_curve.png'), names=()):
    ...
    ax.plot(px, py.mean(1), linewidth=3, color='blue', label='all classes mAP %.3f' % ap[:, 0].mean())
